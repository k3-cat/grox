...
import { CacheControl, D, R2Const } from "@/definitions";
...
app.options(D.GENERAL_KEY_PATH, async (c) => {
	...
	if (!ret) {
		throw new ObjectNotFoundErr("BAK-T", `r2:'${c.var.r2.name}'`, `prefix:'${key}'`);
	}
...
app.get(D.GENERAL_KEY_PATH, async (c) => {
	...
	if (!ret) {
		throw new ObjectNotFoundErr("BAK-G", `r2:'${c.var.r2.name}'`, `file:'${key}'`);
	}
...
app.put(D.GENERAL_KEY_PATH, async (c) => {
	...
	const ret = await c.var.r2.get(key, c.req.raw.headers);
	if (!ret) {
		throw new ObjectNotFoundErr("BAK-P", `r2:'${c.var.r2.name}'`, `file:'${key}'`);
	}
